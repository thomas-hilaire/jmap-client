...
describe('The Client class', function() {
  ...
  describe('The destroyMessage method', function() {
    ...
    it('should resolve the promise with nothing is the message was destroyed', function(done) {
      var client = defaultClient();
    ...
    it('should throw an error if the given id is not in response.destroyed', function(done) {
      ...
      client.destroyMessage('the id').then(null, function(err) {
        expect(err).to.be.an.instanceof(Error);
        expect(err.message).to.equal('Failed to destroy the id, reason is expected message');
        done();
      });
    });

    it('should throw an error if the given id is neither in response.destroyed nor response.notDestroyed', function(done) {
      var client = defaultClient();
      ...
      client.destroyMessage('the id').then(null, function(err) {
        expect(err).to.be.an.instanceof(Error);
        expect(err.message).to.equal('Failed to destroy the id, reason is missing');
        done();
      });
    });
